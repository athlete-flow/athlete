class Framework {
  ...
  #publicFramework = {
    ...
    injectFactory: (token, dependencies) => this.injectFactory(token, dependencies).#publicFramework,
  ...
  buildContainer() {
    ...
    for (const provider of this.#modulesGraph.values()) provider.instantiate(this.#modulesGraph).export(this.#injector);
