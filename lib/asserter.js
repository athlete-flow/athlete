class Asserter {
  ...
  throwIfHasUnknownTokens(graph, dependencies) {
    for (const dependency of dependencies)
      if (!graph.has(dependency)) throw new Error(`[ ${this.#formName(dependency)} ] has no injection.`);
  }
  ...
  throwIfNotValidGraph(graph) {
    this.throwIfHasUnknownToken(graph);
    this.throwIfHasCyclicDependencies(graph);
  }
}
